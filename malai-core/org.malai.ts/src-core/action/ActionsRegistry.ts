...
namespace malai {
    ...
    export class ActionsRegistry {
         ...
         */
        public unregisterActions(action : Action) {
            //FIXME
         ...
         */
        public addAction(action : Action, actionHandler : ActionHandler) {
            //FIXME
            // {
            //     if(action != null && actionHandler != null && !/* contains */(this.actions.indexOf(<any>(action)) >= 0) && (this.sizeMax > 0 || action.getRegistrationPolicy() === Action.RegistrationPolicy.UNLIMITED)) {
            //         this.unregisterActions(action);
            //         if(/* size */(<number>this.actions.length) >= this.sizeMax) {
            //             this.actions.stream().filter((act) => act.getRegistrationPolicy() !== Action.RegistrationPolicy.UNLIMITED).findFirst().ifPresent((act) => {
            ...
            //         };
            //         if(action != null && (action["__interfaces"] != null && action["__interfaces"].indexOf("Undoable") >= 0 || action.constructor != null && action.constructor["__interfaces"] != null && action.constructor["__interfaces"].indexOf("Undoable") >= 0)) {
            //             UndoCollector.INSTANCE_$LI$().add(<Undoable><any>action, actionHandler);
            //         }
            //     }
            // };
        }
         ...
         */
        public addHandler(handler : ActionHandler) {
            if(handler != null) {
                {
        ...
        public setSizeMax(newSizeMax : number) {
            ...
            //             if(/* get */this.actions[i].getRegistrationPolicy() !== Action.RegistrationPolicy.UNLIMITED) {
            //                 /* remove */this.actions.splice(i, 1).flush();
            //                 nb++;
            //             } else {
            //                 i++;
    ...
    ActionsRegistry["__class"] = "malai.ActionsRegistry";

}


malai.ActionsRegistry.INSTANCE_$LI$();
