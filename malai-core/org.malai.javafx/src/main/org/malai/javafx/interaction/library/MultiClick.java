	...
	protected void initStateMachine() {
		...
		new PressureTransition(initState, pressed) {
			...
			public void action() {
				...
				MultiClick.this.points.add(new Point2D.Double(this.event.getX(), this.event.getY()));
				currentPosition = new Point2D.Double(this.event.getX(), this.event.getY());
			}
		...
		new PressureTransition(released, pressed) {
			...
			public void action() {
				MultiClick.this.points.add(new Point2D.Double(this.event.getX(), this.event.getY()));
				currentPosition = new Point.Double(this.event.getX(), this.event.getY());
			}
		...
		new MoveTransition(released, released) {
			@Override
			public void action() {
				currentPosition.setLocation(this.event.getX(), this.event.getY());
			}
		...
		new PressureTransition(released, ended) {
			...
			@Override
			public boolean isGuardRespected() {
				return this.event.getButton() != MouseButton.PRIMARY  & this.hid == MultiClick.this.getLastHIDUsed() && MultiClick.this.points.size() + 1 >= MultiClick.this.minPoints;
			}
		};
		...
		new PressureTransition(released, aborted) {
			@Override
			public boolean isGuardRespected() {
				return this.event.getButton() != MouseButton.PRIMARY && this.hid == MultiClick.this.getLastHIDUsed() && MultiClick.this.points.size() < MultiClick.this.minPoints;
			}
		};
	}
