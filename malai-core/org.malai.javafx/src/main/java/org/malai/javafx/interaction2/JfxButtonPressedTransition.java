...
public class JfxButtonPressedTransition extends JfxWidgetTransition<Button> {
	public JfxButtonPressedTransition(final JfxInteraction<?, Button> interaction, final OutputState<Event> srcState, final InputState<Event> tgtState) {
	...
	protected void action(final Event event) {
		if(event instanceof ActionEvent) {
			interaction.processButtonData(event.getSource());
		}
	}
}
