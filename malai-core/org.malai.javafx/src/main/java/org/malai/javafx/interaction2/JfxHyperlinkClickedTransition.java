...
public class JfxHyperlinkClickedTransition extends JfxWidgetTransition<Hyperlink> {
	public JfxHyperlinkClickedTransition(final JfxInteraction<?, Hyperlink> interaction, final OutputState<Event> srcState, final InputState<Event> tgtState) {
	...
	protected void action(final Event event) {
		if(event instanceof ActionEvent) {
			interaction.processHyperlinkData(event.getSource());
		}
	}
}
