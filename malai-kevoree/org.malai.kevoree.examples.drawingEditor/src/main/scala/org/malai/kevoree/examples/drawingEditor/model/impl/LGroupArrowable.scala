...
import org.malai.kevoree.examples.drawingEditor.model.interfaces.{ILine, IArrow, IGroup}
...
protected trait LGroupArrowable extends IGroup {
  ...
  private def firstIArrowable = getShapes.find {
    shape => shape.isArrowable
  }
  ...
  override def isArrowable() = getShapes.exists {
    shape => shape.isArrowable
  }
