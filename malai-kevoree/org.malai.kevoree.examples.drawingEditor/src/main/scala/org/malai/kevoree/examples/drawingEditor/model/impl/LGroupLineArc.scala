...
import org.malai.kevoree.examples.drawingEditor.model.interfaces.{ILineArcShape, IGroup}
...
protected trait LGroupLineArc extends IGroup {
  ...
  private def firstLineArc = getShapes.find {
    shape => shape.isInstanceOf[ILineArcShape]
  }
  ...
  override def isRoundCorner() = getShapes.exists {
    sh => sh.isInstanceOf[ILineArcShape] && sh.asInstanceOf[ILineArcShape].isRoundCorner
  }
}
